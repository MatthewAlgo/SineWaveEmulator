...
void buildvec() {
	...
	for (size_t* i{ new size_t{0} }; *i < NOS; *i = *i + 1) {
		valuevector[static_cast<sf::Int16>(*i)] = static_cast<sf::Int16>(sin(*valueforsin) * 15000);
		...
		for (;;) {
			for (size_t* i{ new size_t{0} }; *i < 26; *i = *i + 1) {
				if (sf::Keyboard::isKeyPressed(static_cast<sf::Keyboard::Key>(*i))) {
					...
					//std::cout << "CURRENT KEY PRESSED: " << static_cast<char>((*i) + 'A') << " CORRESPONDS TO SAMPLE RATE: " << 20000 + *i * RATE<<"\n";
					SOUND->setBuffer(*Buffer);
					SOUND->play();
				}
			}
